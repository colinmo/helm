...
func (snow *SNOWStruct) Init(baseRedirect string, accessToken string, refreshToken string, expiration time.Time) {
...
func (snow *SNOWStruct) Authenticate(w http.ResponseWriter, r *http.Request) {
	...
	if query.Get("code") != "" {
		...
		if err != nil {
			...
			snow.RefreshToken = MSToken.RefreshToken
			...
			snow.AccessToken = MSToken.AccessToken
		}
	}
}
				...
				snow.G.Do(
					...
					func() (interface{}, error) {
						...
						for page := 1; page < 200; page++ {
							tasksResponse, _ := snow.GetMyRequestsForPage(page)
							if len(tasksResponse.BusinessObjects) > 0 {
								for _, x := range tasksResponse.BusinessObjects {
									...
									snow.LoggedIncidents = append(snow.LoggedIncidents, row)
								}
							}
			...
			go func() {
				snow.G.Do(
					...
					func() (interface{}, error) {
						...
						for page := 1; page < 200; page++ {
							tasksResponse, _ := snow.GetMyTeamIncidentsInGSMForPage(page)
							if len(tasksResponse.BusinessObjects) > 0 {
								for _, x := range tasksResponse.BusinessObjects {
									if x.Fields[6].Value == snow.UserID {
										continue
									}
									row := TaskResponseStruct{
										ID: x.BusObPublicId,
									...
									if row.OwnerID == "" {
										snow.TeamIncidents = append(snow.TeamIncidents, row)
									}
								}
							}
							if len(tasksResponse.BusinessObjects) != 200 {
...
func (snow *SNOWStruct) GetMyIncidentsForPage(page int) ([]SnowIncident, error) {
	r, err := snow.SearchSnowFor(
		"incident", // table
		[]string{"number", "short_description", "sys_id", "priority"},               // fields to return
		map[string]string{"assigned_to": AppPreferences.SnowUser, "close_code": ""}, // filters
		page,
	)
	return r, err
}
...
type SnowResponse struct {
	Results []SnowResponseIncidents `json:"results"`
}
...
func (snow *SNOWStruct) SearchSnowFor(table string, fields []string, filter map[string]string, page int) ([]SnowIncident, error) {
	...
	result, err := snow.getStuffFromURL(
		...
		"/api/now/table/"+table,
		fmt.Sprintf(
			"sysparam_fields=%s&sysparam_query=%s&sysparam_offset=%d",
			strings.Join(fields, ","),
	...
	if err == nil {
		defer result.Close()
		_ = json.NewDecoder(result).Decode(&incidentsResponse)
